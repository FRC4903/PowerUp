...
//	Servo *leftServo = new Servo(3);
    ...
    Robot() : talonRight1(1), talonRight2(2),
              ...
              cubeLiftInductiveTop(0), cubeLiftInductiveBottom(1), hookTalon1(9), hookTalon2(10),
              hookLiftInductiveTop(2), hookLiftInductiveBottom(3) {
...
//		(inches*inches * a) + b*inches + c
    ...
    void AutonomousPeriodic() {
        if (gameData.length() > 0 && !done) {
            ...
            int endGame = SmartDashboard::GetNumber("DB/Slider 1",
                                                    1.0); // 0 - Vault, 1 - Switch, 2 - farSwitch, 3 - scale
